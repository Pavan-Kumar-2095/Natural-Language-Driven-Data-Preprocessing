...
def match_column(text,columns):
    ...
    best=rf_process.extractOne(text,columns_lower)

    if best and best[1]>=60:
        return columns[columns_lower.index(best[0])]
...
async def process_operation(payload:dict):
    ...
    operation=label_encoder.inverse_transform([idx])[0]

    if confidence<0.8:
        return JSONResponse({"error":"Low confidence prediction"},status_code=400)
    ...
    try:
        ...
        if operation=="REMOVE_DUPLICATES_ROWS":
            ...
            if dtype=="int":
                ...
                df[matched_column]=pd.to_datetime(df[matched_column],errors="coerce")
...
async def get_info(file_path:str):
    ...
    return {
        "info":clean_nan(info),
        "describe":clean_nan(describe)
    }

@app.get("/get_correlation")
async def get_correlation(file_path: str):
